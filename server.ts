...
// Endpoint 2: AI lesson generator
app.post("/api/lesson/generate", async (req, res) => {
  ...
  // We want to generate a LessonVersion structure
  const gemini = getGeminiClient();
  const openai = getOpenAIClient();

  // Prompt detailing the exact structure we need
  const systemPrompt = `You are a professional, expert English teacher specialized in Differentiated Instruction.
...
- The returned correctEnglish1 and correctEnglish2 must be fully in line with standard academic English.`;
  ...
  try {
    if (gemini) {
      // Use standard Gemini call (cost-saving flash model)
      console.log("Calling Gemini API for lesson generation...");
      const contentsList: any[] = [];
      if (geminiFilePart) {
        contentsList.push(geminiFilePart);
      }
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          ...
          { role: "user", content: cleanMaterial 
            ? `Create a comprehensive differentiated English lesson based on this uploaded material: "${cleanMaterial}"`
            : `Create a comprehensive differentiated English lesson on the topic: "${cleanTopic}"` 
          },
        ],
      ...
      if (!content) {
        throw new Error("Empty content back from OpenAI.");
      }
      ...
      // No keys loaded, generate a neat simulated lesson based on the topic
      console.log("No AI key configured. Creating a beautiful local simulation of the lesson...");
      
      // We simulate a lesson structure
...
// Endpoint 3: AI Translation Grader
app.post("/api/lesson/grade-translation", async (req, res) => {
  ...
  const gradingPrompt = `You are a professional, rigorous English grammar grader and tutor.
...
JSON Specifications:
{
  ...
  try {
    if (gemini) {
      ...
      const content = completion.choices[0].message.content;
      if (!content) {
        throw new Error("Empty grader result.");
      }
...
// Fallback generator for simulated lessons
function createSimulatedLesson(topic: string): any {
  return {
...
// Simple rule-based translation grader (high reliability fallback)
function localGrader(student1: string, student2: string, ref1: string, ref2: string, level: string): any {
  ...
  // Calculate some simple keyword match metric
  ...
  // Advanced grammar syntax checks
  if (level === "advanced") {
    // Check for negative inversion or conditional
    const invertedWordCheck = ["seldom", "never", "only", "not until", "scarcely", "hardly", "were", "had"].some(w => 
      cleanAns1.includes(w) || cleanAns2.includes(w)
    );
    if (!invertedWordCheck) {
      score = Math.max(30, score - 20); // deduct for missing specialty syntax
    } else {
      score = Math.min(100, score + 10);
    }
  }

  // Adjust score if student attempts are empty
  ...
  return {
    ...
    grammarFeedback: level === "advanced" 
